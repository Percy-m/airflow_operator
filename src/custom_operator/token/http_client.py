"""Shared requests wrapper with retry and API error mapping."""
...
from custom_operator.token.exceptions import CustomOperatorApiError

log = logging.getLogger(__name__)


class HttpClient:
    """Synchronous HTTP client used by custom operators and triggers."""
        ...
        *,
        ...
        for attempt in range(attempts):
            ...
            try:
                ...
                log.info(
                    "HTTP response method=%s url=%s status_code=%s cost_ms=%s body=%s",
                    method,
                    url,
        ...
        raise CustomOperatorApiError(f"HTTP {method} {url} failed: {last_error}", retryable=True)
