"""Airflow Connection parsing for compute operator runtime config."""
...
def resolve_connection_config(
    ...
    *,
    ...
    request_timeout: int,
    verify: bool,
    auth_error_cls: type[Exception] = AiDatalakeAuthError,
    service_name: str = "AiDatalake",
) -> ConnectionConfig:
    """Resolve endpoint and static token from direct config or Airflow Connection."""
    ...
    if not resolved_token:
        raise auth_error_cls(f"{service_name} token must be configured in Connection password")
